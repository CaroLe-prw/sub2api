...
func newSimpleModeGroupRouter(svc *stubAdminService) *gin.Engine {
	...
	r.PUT("/groups/batch-model-allowlist", h.BatchSetModelAllowlist)
...
func TestGroupHandlerSimpleModeBlocksAdvancedOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{http.MethodPut, "/groups/batch-model-allowlist", `{"group_ids":[1],"model_allowlist":{"enabled":true,"models":["gpt-*"]}}`},
