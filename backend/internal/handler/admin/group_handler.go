...
type BatchSetGroupModelAllowlistRequest struct {
	GroupIDs       []int64                     `json:"group_ids" binding:"required,min=1"`
	ModelAllowlist service.GroupModelAllowlist `json:"model_allowlist"`
}

// BatchSetModelAllowlist updates the model allowlist for
// several selected groups.
// PUT /api/v1/admin/groups/batch-model-allowlist
func (h *GroupHandler) BatchSetModelAllowlist(c *gin.Context) {
	if h.rejectUnsupportedSimpleModeOperation(c, "advanced") {
		return
	}
	var req BatchSetGroupModelAllowlistRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	affected, err := h.adminService.BatchSetGroupModelAllowlist(c.Request.Context(), req.GroupIDs, req.ModelAllowlist)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
