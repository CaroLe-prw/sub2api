...
func TestToModelPlazaGroupDTO_UserRateAndFieldWhitelist(t *testing.T) {
	...
	timePricing := pricing["time_pricing"].(map[string]any)
	require.Equal(t, "Asia/Shanghai", timePricing["timezone"])
	periods := timePricing["periods"].([]any)
	require.Len(t, periods, 2)
	require.Equal(t, "09:00", periods[0].(map[string]any)["start_time"])
	require.Equal(t, "16:00", periods[1].(map[string]any)["end_time"])
