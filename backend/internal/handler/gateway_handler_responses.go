	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				action := fs.HandleFailoverError(requestCtx, h.gatewayService, account.ID, account.Platform, failoverErr.SameAccountRetryLimit(account.GetPoolModeRetryCount()), failoverErr)
				switch action {
				case FailoverContinue:
					continue
