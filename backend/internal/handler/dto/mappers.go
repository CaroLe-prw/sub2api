...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		GroupID:                      k.GroupID,
		Status:                       k.Status,
		SchedulingStatus:             k.SchedulingStatus(),
		MaxGroupRateMultiplier:       k.MaxGroupRateMultiplier,
		EffectiveGroupRateMultiplier: k.EffectiveGroupRateMultiplier(),
...
func redactAccountManagedExtra(extra map[string]any) map[string]any {
	...
	for key, value := range extra {
		...
		case service.IsOpenAICodexTicketPrivateExtraKey(key):
			continue
		case key == service.NewAPISyncIdentityExtraKey:
			continue
		case key == service.NewAPIUserAccessTokenExtraKey, key == service.NewAPIAPIKeyExtraKey:
...
func UsageLogFromServiceAdmin(l *service.UsageLog) *AdminUsageLog {
	...
	return &AdminUsageLog{
		...
		ResponseDiagnostics:     l.ResponseDiagnostics,
