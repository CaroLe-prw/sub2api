...
const apiKeyAuthSnapshotVersion = 25 // v25: include Codex model manifest configuration with local pricing and Fast policy fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			MaxAccountCostMultiplier:        apiKey.Group.MaxAccountCostMultiplier,
		}
	}
	return snapshot
}
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			MaxAccountCostMultiplier:        snapshot.Group.MaxAccountCostMultiplier,
		}
	}
