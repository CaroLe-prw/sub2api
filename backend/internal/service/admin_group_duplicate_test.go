...
func cloneGroupForDuplicateTest(group *Group) *Group {
	...
	cloned.ModelMapping = cloneGroupModelMapping(group.ModelMapping)
...
func TestDuplicateGroupCopiesConfigurationDeeplyAndResetsRuntimeState(t *testing.T) {
	...
	source := &Group{
		...
		RateMultiplier:               1.75,
		MaxAccountCostMultiplier:     groupDuplicateTestPointer(0.05),
		OpenAISchedulerProfile:       GroupOpenAISchedulerProfileCustom,
		OpenAISchedulerConfig:        DefaultGroupOpenAISchedulerConfig(),
		...
		ModelMapping:                    map[string]string{"luna": "terra"},
	...
	require.Equal(t, source.MaxAccountCostMultiplier, duplicate.MaxAccountCostMultiplier)
	require.Equal(t, source.OpenAISchedulerProfile, duplicate.OpenAISchedulerProfile)
	require.Equal(t, source.OpenAISchedulerConfig, duplicate.OpenAISchedulerConfig)
	...
	require.Equal(t, source.ModelMapping, duplicate.ModelMapping)
