	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			if !clientCanceled && autoContextCompactionInjected {
				...
				if rejected {
					s.recordOpenAIContextCompactionObservation(ctx, account, false, resp.StatusCode, extractUpstreamErrorMessage(probeBody))
					body = retryBody
					autoContextCompactionInjected = false
					setOpenAIAutoContextCompactionState(c, false, false)
					logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Retrying passthrough request without unsupported context_management (account: %s)", account.Name)
					continue
				}
			}
			...
			if openAIContextCompactionHTTPShouldFailover(c, resp.StatusCode, upstreamMsg, probeBody) ||
		...
		if reqStream {
			result, handleErr := s.handleStreamingResponsePassthroughWithFirstOutput(
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	return s.newOpenAIAccountFailoverError(
		...
		!shouldDisable && ((account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode)) ||
			shouldRetryOpenAINonOAuthCapacityOnSameAccount(account, resp.StatusCode, upstreamMsg, body)),
...
func openAIStreamEventIsMetadata(eventType string) bool {
	...
	case "response.created", "response.in_progress", "response.queued", "codex.rate_limits", "codex.response.metadata", "keepalive":
		return true
	default:
		return false
	}
...
func openAIStreamDataStartsClientOutput(data, eventType string, contexts ...*gin.Context) bool {
...
// Structural Responses events prove that the upstream request is making
// progress and therefore disarm the first-output timeout. Empty reasoning or
// content containers are still kept attempt-local: they are not useful client
// output and must not prevent a safe failover if the stream subsequently
// fails.
func openAIStreamDataStartsFirstOutputProgress(data, eventType string, contexts ...*gin.Context) bool {
	return openAIStreamDataStartsClientOutput(data, eventType, contexts...) ||
		openAIStreamEventIsEmptyStructure(data, eventType)
}
...
func openAIStreamFailedEventSemanticStatus(payload []byte, message string) int {
	...
	case isOpenAIStreamRetryableUpstreamFailure(payload, message):
...
// isOpenAIStreamRetryableUpstreamFailure recognizes provider-side failures that
// are sometimes mislabeled as invalid_request_error inside an HTTP 200 SSE
// response.failed event. The message/code is authoritative here: treating the
// outer error type as a client fault made the gateway flush the terminal event
// and prevented both same-account retry and account failover.
func isOpenAIStreamRetryableUpstreamFailure(payload []byte, message string) bool {
	code := openAIStreamFailedEventErrorCode(payload)
	errType := strings.ToLower(strings.TrimSpace(gjson.GetBytes(payload, "response.error.type").String()))
	if errType == "" {
		errType = strings.ToLower(strings.TrimSpace(gjson.GetBytes(payload, "error.type").String()))
	...
	switch code {
	case "server_error", "api_error", "upstream_error", "internal_error", "internal_server_error",
		"service_unavailable", "server_is_overloaded", "slow_down", "timeout", "gateway_timeout":
		return true
	}
	switch errType {
	case "server_error", "api_error", "upstream_error", "internal_error", "internal_server_error",
		"service_unavailable_error", "timeout_error":
		return true
	}
...
func openAIStreamFailedEventShouldFailover(payload []byte, message string, contexts ...*gin.Context) bool {
	...
	if isOpenAIStreamRetryableUpstreamFailure(payload, message) {
		return true
	}
	code := strings.ToLower(strings.TrimSpace(gjson.GetBytes(payload, "response.error.code").String()))
	if code == "" {
...
func openAIStreamFailedEventRetryableOnSameAccount(account *Account, payload []byte, message string) bool {
	...
	// Explicit stream capacity-shed codes are request-scoped and use the
	// bounded same-account retry for every account type.
	if isOpenAIUpstreamCapacityShedEvent(payload) {
		return true
	}
	// For message-only capacity failures, non-OAuth accounts retry immediately.
	...
	return account.IsPoolModeRetryableStatus(semanticStatus) ||
		...
		isOpenAIStreamRetryableUpstreamFailure(payload, message)
}

func openAIStreamUpstreamErrorMetadata(payload []byte) string {
	if len(payload) == 0 || !gjson.ValidBytes(payload) {
		return ""
	}
	...
	metadata := gin.H{}
	if errType := strings.TrimSpace(gjson.Get(errorValue.Raw, "type").String()); errType != "" {
		metadata["type"] = errType
	}
	if code := strings.TrimSpace(gjson.Get(errorValue.Raw, "code").String()); code != "" {
		metadata["code"] = code
	}
	if param := strings.TrimSpace(gjson.Get(errorValue.Raw, "param").String()); param != "" {
		metadata["param"] = param
	}
	if len(metadata) == 0 {
		return ""
	}
	body, err := json.Marshal(gin.H{"error": metadata})
	if err != nil {
		return ""
	}
	return string(body)
...
func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamError(
	...
	statusCode := openAIStreamFailureStatus(payload, message)
	// Persist non-sensitive type/code/param metadata even when raw upstream-body
	// logging is disabled. This keeps HTTP-200 response.failed incidents
	// diagnosable without storing prompts, outputs, or full response envelopes.
	detail := openAIStreamUpstreamErrorMetadata(payload)
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithModel(
	...
	retryableIfNoOtherAccount := shouldRetryOpenAIOAuthCapacityOnSameAccount(account, http.StatusBadRequest, message, payload)
	...
	if !failoverErr.IsCredentialFailure() && !failoverErr.RequestScopedTransient {
		// Preserve the generic envelope for unclassified stream failures; typed
		// access/capacity failures retain the provider payload.
		failoverErr.ResponseBody = body
	}
	failoverErr.ExplicitUpstreamResponse = len(bytes.TrimSpace(payload)) > 0
	failoverErr.RequestScopedTransient = failoverErr.RequestScopedTransient || isOpenAIStreamRequestScopedCapacityError(payload, message)
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	return s.handleStreamingResponsePassthroughWithFirstOutput(
		ctx, resp, c, account, startTime, originalModel, mappedModel, "", firstOutputTimeout,
	)
}

func (s *OpenAIGatewayService) handleStreamingResponsePassthroughWithFirstOutput(
	ctx context.Context,
	resp *http.Response,
	c *gin.Context,
	account *Account,
	...
	firstOutputTimeoutFired := make(chan struct{})
	if firstOutputTimeout > 0 {
	...
	for documentScanner.Scan() {
		...
		lineStartsFirstOutputProgress := false
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
				...
				if !outputStarted {
					...
					if !cyberHit {
						if eventType == "error" {
							...
							shouldFailover = openAIStreamFailedEventShouldFailover(dataBytes, failedMessage, c)
						}
					}
					if shouldFailover {
			...
			lineStartsFirstOutputProgress = forceFlushFailedEvent || openAIStreamDataStartsFirstOutputProgress(trimmedData, eventType, c)
			if lineStartsFirstOutputProgress && !firstOutputProgressObserved {
	...
	if !firstOutputProgressObserved && firstOutputTimedOut() {
