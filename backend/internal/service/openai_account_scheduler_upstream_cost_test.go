...
func TestOpenAISchedulingRateCalibratesFreshRateAndFallsBackToAccountRate(t *testing.T) {
	...
	rate, ok := openAISchedulingRate(account, now, floatPtr(1))
	require.True(t, ok)
	require.InDelta(t, 0.03, rate, 1e-12)

	rate, ok = openAISchedulingRate(account, now.Add(2*time.Hour), floatPtr(1))
	require.True(t, ok)
...
func TestBuildOpenAIAccountSchedulerScoreSnapshotUpstreamCostIsExactNoOpWithoutSignal(t *testing.T) {
	...
	baseline := buildOpenAIAccountSchedulerScoreSnapshot(accounts, loadMap, weights, false, floatPtr(defaultOpenAIOAuthSchedulingRateMultiplier), nil)
	weights.UpstreamCost = 1.5
	withCost := buildOpenAIAccountSchedulerScoreSnapshot(accounts, loadMap, weights, false, floatPtr(defaultOpenAIOAuthSchedulingRateMultiplier), nil)
...
func TestBuildOpenAIAccountSchedulerScoreSnapshotUsesUpstreamCostSignal(t *testing.T) {
	...
	scores := buildOpenAIAccountSchedulerScoreSnapshot(accounts, nil, weights, false, floatPtr(defaultOpenAIOAuthSchedulingRateMultiplier), nil)
