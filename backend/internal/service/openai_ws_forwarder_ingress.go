...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			uncoveredToolOutput := toolOutputCoverage.HasFunctionCallOutput && !toolOutputCoverage.ContextCoversAllCallIDs
			...
			needsBridgeReplay = needsBridgeReplay || currentBridgePayload.previousResponseID != "" ||
				uncoveredToolOutput
			...
			if readErr != nil {
				...
				if firstOutputTimeout > 0 && firstTokenMs == nil && ctx.Err() == nil && time.Until(turnStart.Add(firstOutputTimeout)) <= 5*time.Millisecond {
					...
					return nil, s.newOpenAIFirstOutputTimeoutError(
						ctx, c, account, wsProxyID, wsProxyName, turnStart, originalModel, "", firstOutputTimeout,
