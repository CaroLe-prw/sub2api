...
func TestPrepareOpenAIWSHTTPBridgeBodyStripsWSFields(t *testing.T) {
	...
	continuationOnly, err := prepareOpenAIWSHTTPBridgeBody(nil, []byte(`{"type":"response.create","model":"gpt-5","previous_response_id":"resp_only"}`))
	require.NoError(t, err)
...
func TestProxyOpenAIWSHTTPBridgeTurnHandlesCapacityShedBeforeAndAfterFailoverBoundary(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{
			name:       "turn2_error_frame_is_rewritten_for_client",
			turn:       2,
		...
		{
			// Before any downstream output, a first-turn capacity failure remains
			// eligible for the gateway's bounded same-account failover path.
			name:         "turn1_bare_response_failed_returns_failover",
			turn:         1,
...
func TestOpenAIWSHTTPBridgeRelaysSSEFramesAsWebSocketMessages(t *testing.T) {
	...
	payload := []byte(`{"type":"response.create","generate":true,"model":"gpt-5","stream":true,"client_metadata":{"ws_request_header_x_openai_internal_codex_responses_lite":"true"},"parallel_tool_calls":true,"tools":[{"type":"function","name":"shell","parameters":{"type":"object"}}],"input":"hi"}`)
...
func TestProxyResponsesWebSocketFromClientForGrokUsesXAIHTTPBridgeAndPreservesMappedModels(t *testing.T) {
	...
	require.Equal(t, xai.DefaultTextModel, gjson.GetBytes(upstream.bodies[0], "model").String())
...
func TestProxyOpenAIWSHTTPBridgeTurn_KeepsRequestedTierWhenUpstreamReportsDefault(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestProxyOpenAIWSHTTPBridgeTurn_KeepsNormalizedFastTierWhenUpstreamReportsDefault(t *testing.T) {
	gin.SetMode(gin.TestMode)

	// 客户端别名 fast 归一化为 priority；上游回显的 default 单独记录。
	sse := strings.Join([]string{
		`data: {"type":"response.completed","response":{"id":"resp_tier2","model":"gpt-5.5","status":"completed","service_tier":"default","usage":{"input_tokens":1,"output_tokens":1}}}`,
		``,
	}, "\n")
