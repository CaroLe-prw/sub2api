...
type cachedOpenAIAdvancedSchedulerSetting struct {
	...
	schedulerObservabilityEnabled       bool
	schedulerObservabilityMaxTraces     int
	schedulerObservabilityRetentionDays int
	lbTopKOverride                      int
...
type openAIPinnedRetryAccountContextKey struct{}
...
type openAIAccountRuntimeStats struct {
	...
	historyAccountIDs map[int64]struct{}
	historyModelKeys  map[openAIAccountModelKey]struct{}
}
...
func (s *openAIAccountRuntimeStats) loadOrCreate(accountID int64) *openAIAccountRuntimeStat {
	...
	stat.probeTTFTBits.Store(math.Float64bits(math.NaN()))
...
func (s *openAIAccountRuntimeStats) report(accountID int64, success bool, firstTokenMs *int) {
	s.reportTraffic(accountID, "", success, firstTokenMs)
}

func (s *openAIAccountRuntimeStats) reportTraffic(accountID int64, model string, success bool, firstTokenMs *int) {
...
func (s *openAIAccountRuntimeStats) reportTrafficStat(stat *openAIAccountRuntimeStat, success bool, firstTokenMs *int) {
	if stat == nil {
		return
	}
	const alpha = 0.2
	stat.liveSamples.Add(1)
...
func (s *openAIAccountRuntimeStats) reportProbe(accountID int64, model string, success bool, firstTokenMs *int) {
	if s == nil || accountID <= 0 {
		return
	}
	for _, stat := range []*openAIAccountRuntimeStat{s.loadOrCreate(accountID), s.loadOrCreateModel(accountID, model)} {
		if stat == nil {
			continue
		}
		const alpha = 0.35
		...
		if samples == 1 {
			...
			updateEWMAAtomic(&stat.probeErrorBits, errorSample, alpha)
		}
		updateOptionalEWMAAtomic(&stat.probeTTFTBits, firstTokenMs, alpha)
	}
}

func updateOptionalEWMAAtomic(target *atomic.Uint64, sample *int, alpha float64) {
...
func (s *openAIAccountRuntimeStats) loadOrCreateModel(accountID int64, model string) *openAIAccountRuntimeStat {
	...
	result, _ := actual.(*openAIAccountRuntimeStat)
	return result
}
...
func (s *openAIAccountRuntimeStats) snapshotStat(stat *openAIAccountRuntimeStat) (float64, float64, bool) {
	return combineOpenAIHealthSignals(s.signalsForStat(stat))
}

func (s *openAIAccountRuntimeStats) signalsForStat(stat *openAIAccountRuntimeStat) []openAIHealthSignal {
	if stat == nil {
		return nil
	}
	signals := make([]openAIHealthSignal, 0, 3)
	if samples := stat.liveSamples.Load(); samples > 0 {
		signals = append(signals, openAIHealthSignal{
			errorRate: clamp01(math.Float64frombits(stat.errorRateEWMABits.Load())),
			ttft:      math.Float64frombits(stat.ttftEWMABits.Load()), hasTTFT: !math.IsNaN(math.Float64frombits(stat.ttftEWMABits.Load())),
	...
	if samples := stat.probeSamples.Load(); samples > 0 && openAIProbeSignalIsFresh(stat, time.Now()) {
		signals = append(signals, openAIHealthSignal{
			...
			weight: math.Min(float64(samples), 4),
		})
	}
	if samples := stat.historySamples.Load(); samples > 0 && stat.historyGeneration.Load() == s.historyGen.Load() {
		signals = append(signals, openAIHealthSignal{
			errorRate: clamp01(math.Float64frombits(stat.historyErrorBits.Load())),
			ttft:      math.Float64frombits(stat.historyTTFTBits.Load()), hasTTFT: !math.IsNaN(math.Float64frombits(stat.historyTTFTBits.Load())),
			weight: math.Min(float64(samples), 20),
		})
	}
	return signals
}
...
func combineOpenAIHealthSignals(signals []openAIHealthSignal) (errorRate float64, ttft float64, hasTTFT bool) {
	var weightSum, ttftWeight float64
	for _, signal := range signals {
		if signal.weight <= 0 {
			continue
		}
		errorRate += signal.errorRate * signal.weight
		weightSum += signal.weight
		if signal.hasTTFT && signal.ttft > 0 {
			ttft += signal.ttft * signal.weight
			ttftWeight += signal.weight
		}
	}
	if weightSum > 0 {
		errorRate = clamp01(errorRate / weightSum)
	}
	if ttftWeight > 0 {
		ttft /= ttftWeight
		hasTTFT = true
...
func (s *openAIAccountRuntimeStats) consecutiveErrorCount(accountID int64) int64 {
	if s == nil || accountID <= 0 {
		return 0
	}
	value, ok := s.accounts.Load(accountID)
...
// OpenAISchedulerHealthSnapshot is a bounded recent-history aggregate. Success
// samples come from usage logs; failures are restricted to provider-owned,
...
type openAISchedulerHealthSnapshotRepository interface {
	GetOpenAISchedulerHealthSnapshots(ctx context.Context, since time.Time) ([]OpenAISchedulerHealthSnapshot, error)
}

func (s *openAIAccountRuntimeStats) replaceHistory(snapshots []OpenAISchedulerHealthSnapshot) {
	if s == nil {
		return
	}
	...
	for _, snapshot := range snapshots {
		if snapshot.AccountID <= 0 || snapshot.SuccessCount+snapshot.FailureCount <= 0 {
			continue
		}
		apply := func(stat *openAIAccountRuntimeStat) {
			if stat == nil {
				return
			}
			total := snapshot.SuccessCount + snapshot.FailureCount
			stat.historyErrorBits.Store(math.Float64bits(float64(snapshot.FailureCount) / float64(total)))
			if snapshot.AvgTTFTMs != nil && *snapshot.AvgTTFTMs > 0 {
				stat.historyTTFTBits.Store(math.Float64bits(*snapshot.AvgTTFTMs))
			} else {
				stat.historyTTFTBits.Store(math.Float64bits(math.NaN()))
			}
		...
		if hasModel {
			apply(s.loadOrCreateModel(snapshot.AccountID, snapshot.Model))
			newModelKeys[key] = struct{}{}
		}
		total := accountTotals[snapshot.AccountID]
		total.AccountID = snapshot.AccountID
		total.SuccessCount += snapshot.SuccessCount
		total.FailureCount += snapshot.FailureCount
		if snapshot.AvgTTFTMs != nil && snapshot.SuccessCount > 0 {
			weighted := *snapshot.AvgTTFTMs * float64(snapshot.SuccessCount)
			if total.AvgTTFTMs != nil {
				weighted += *total.AvgTTFTMs * float64(total.SuccessCount-snapshot.SuccessCount)
			}
			value := weighted / float64(total.SuccessCount)
			total.AvgTTFTMs = &value
		}
		accountTotals[snapshot.AccountID] = total
	}
	for accountID, total := range accountTotals {
		stat := s.loadOrCreate(accountID)
		count := total.SuccessCount + total.FailureCount
		stat.historyErrorBits.Store(math.Float64bits(float64(total.FailureCount) / float64(count)))
		if total.AvgTTFTMs != nil {
			stat.historyTTFTBits.Store(math.Float64bits(*total.AvgTTFTMs))
		} else {
			stat.historyTTFTBits.Store(math.Float64bits(math.NaN()))
		}
		stat.historySamples.Store(count)
		stat.historyGeneration.Store(generation)
		newAccountIDs[accountID] = struct{}{}
	}
	for accountID := range s.historyAccountIDs {
		if _, ok := newAccountIDs[accountID]; ok {
			continue
		}
		if value, ok := s.accounts.Load(accountID); ok {
			clearOpenAIHistorySignal(value)
		}
	}
	for key := range s.historyModelKeys {
		if _, ok := newModelKeys[key]; ok {
			continue
		}
	...
	s.historyAccountIDs = newAccountIDs
	s.historyModelKeys = newModelKeys
}

func clearOpenAIHistorySignal(value any) {
	stat, _ := value.(*openAIAccountRuntimeStat)
	if stat == nil {
		return
	}
...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	defer func() {
		...
		decision.StickyEscapeReason = observation.stickyEscapeReason
		if len(observation.candidates) == 0 && selectionResult != nil && selectionResult.Account != nil {
			observation.candidates = []OpenAISchedulerObservabilityCandidate{{
				AccountID:   selectionResult.Account.ID,
				AccountName: selectionResult.Account.Name,
				Rank:        1,
				State:       "selected",
			}}
		}
		decision.Candidates = cloneSchedulerCandidates(observation.candidates)
		s.metrics.recordSelect(decision)
		if s.service != nil {
			s.service.RecordOpenAISchedulerObservabilitySelection(ctx, req, decision, selectionResult, selectionErr)
		}
	}()
	...
	// An established session owns its account while that account remains healthy.
	// Weighted scoring is used for initial placement and failover, not to reshuffle
	// a healthy conversation on every turn.
	{
		...
		if stickyEscapeReason != "" {
			observation.stickyEscapeReason = stickyEscapeReason
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	pinned := req.PinnedAccountID > 0
	if s == nil || s.service == nil || (!pinned && (sessionHash == "" || s.service.cache == nil)) {
		return nil, "", nil
	}

	accountID := req.PinnedAccountID
	if accountID <= 0 {
		accountID = req.StickyAccountID
	}
	clearBinding := func() {
		if !pinned && !req.PreserveStickyBinding {
	...
	if reason, errorRate, ttft, shouldEscape := s.shouldEscapeStickyAccount(accountID, escapeCfg); !pinned && shouldEscape {
		...
		req.observation.recordStickyEscape(accountID, reason)
	...
	if s.service.concurrencyService != nil {
		if !pinned && escapeCfg.enabled && acquireErr == nil && result != nil && !result.Acquired {
			...
			req.observation.recordStickyEscape(accountID, "concurrency_full")
			return nil, "concurrency_full", nil
		}
		return attachSelectionProfitGate(ctx, &AccountSelectionResult{
			Account: account,
...
func (o *openAIAccountScheduleObservation) recordStickyEscape(accountID int64, reason string) {
	if o == nil || strings.TrimSpace(reason) == "" {
		return
	}
...
func (o *openAIAccountScheduleObservation) isStickyEscapeAccount(accountID int64) bool {
...
func (o *openAIAccountScheduleObservation) recordCandidateReason(accountID int64, reason string) {
	if o == nil || accountID <= 0 || strings.TrimSpace(reason) == "" {
		return
	}
	if o.candidateReasons == nil {
		o.candidateReasons = make(map[int64]string)
	}
	o.candidateReasons[accountID] = reason
}

func (o *openAIAccountScheduleObservation) captureCandidates(candidates []openAIAccountCandidateScore) {
	...
	o.candidates = make([]OpenAISchedulerObservabilityCandidate, 0, len(ranked))
...
func openAIWeightedStickyWithinScoreGap(baseScore float64, bestBaseScore float64) bool {
	if math.IsNaN(baseScore) || math.IsInf(baseScore, 0) || math.IsNaN(bestBaseScore) || math.IsInf(bestBaseScore, 0) {
		return false
	}
	if baseScore >= bestBaseScore {
		return true
	}
	scale := math.Max(math.Abs(bestBaseScore), 1)
	return bestBaseScore-baseScore <= scale*openAIWeightedStickyMaxScoreGapRatio
}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	req.observation.captureCandidates(candidates)
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	if req.RequireCompact {
		...
		if len(plan.staleSnapshotCompactRetry) > 0 && s.service.schedulerSnapshot != nil {
			selectionOrder = append(selectionOrder, sortOpenAICompactRetryCandidates(selectableCandidates(plan.staleSnapshotCompactRetry))...)
		}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	if len(accounts) == 0 {
		return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, openAISelectionFilterStats{}, "")
	}
	...
	if len(accounts) == 0 {
		return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, openAISelectionFilterStats{}, "grok_free_quota_soft_gate")
	}
	...
	if req.Platform == PlatformGrok {
		...
		if len(filtered) == 0 && len(accounts) > 0 {
			return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, openAISelectionFilterStats{}, "grok_team_model_rate_limit")
		}
		if filtered != nil {
			accounts = filtered
		}
		...
		if len(modelFiltered) == 0 && len(accounts) > 0 {
			return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, openAISelectionFilterStats{}, "grok_model_quota_block")
		}
	...
	if len(filtered) == 0 {
		return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, filterStats, "")
	}
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	if len(attempt.selectionOrder) == 0 {
		return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, attempt.compactBlocked, filterStats, "selection_order_empty")
	}
	...
	for pass := 0; pass < passes; pass++ {
		...
		for _, candidate := range attempt.selectionOrder {
			...
			if !s.consumeOpenAISelectionDBRecheck(budget) {
				return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked, filterStats, "selection_order_exhausted")
			}
			fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.GroupID, req.Platform, req.RequestedModel, false, req.RequiredCapability)
			if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
				continue
...
func (s *defaultOpenAIAccountScheduler) ReportResult(accountID int64, model string, success bool, firstTokenMs *int) {
	...
	s.stats.reportTraffic(accountID, model, success, firstTokenMs)
}
...
func (s *OpenAIGatewayService) openAIAdvancedSchedulerRuntimeSettings(ctx context.Context) openAIAdvancedSchedulerRuntimeSettings {
	if cached, ok := openAIAdvancedSchedulerSettingCache.Load().(*cachedOpenAIAdvancedSchedulerSetting); ok && cached != nil {
		if time.Now().UnixNano() < cached.expiresAt {
			return openAIAdvancedSchedulerRuntimeSettings{
				...
				schedulerObservabilityEnabled:       cached.schedulerObservabilityEnabled,
				schedulerObservabilityMaxTraces:     cached.schedulerObservabilityMaxTraces,
				schedulerObservabilityRetentionDays: cached.schedulerObservabilityRetentionDays,
	...
	result, _, _ := openAIAdvancedSchedulerSettingSF.Do(openAIAdvancedSchedulerSettingKey, func() (any, error) {
		...
		schedulerObservabilityEnabled := true
		schedulerObservabilityMaxTraces := DefaultOpenAISchedulerObservabilityMaxTraces
		schedulerObservabilityRetentionDays := DefaultOpenAISchedulerObservabilityRetentionDays
		lbTopKOverride := 0
		weightOverrides := map[string]float64{}
		...
		if repo := s.openAIAdvancedSchedulerSettingRepo(); repo != nil {
			...
			if values, err := repo.GetMultiple(dbCtx, openAIAdvancedSchedulerRuntimeSettingKeys()); err == nil {
				...
				schedulerObservabilityEnabled = !isFalseSettingValue(values[SettingKeyOpenAISchedulerObservabilityEnabled])
				schedulerObservabilityMaxTraces = parseOpenAISchedulerObservabilityMaxTraces(values[SettingKeyOpenAISchedulerObservabilityMaxTraces])
				schedulerObservabilityRetentionDays = parseOpenAISchedulerObservabilityRetentionDays(values[SettingKeyOpenAISchedulerObservabilityRetentionDays])
				...
				schedulerObservabilityEnabled = !isFalseSettingValue(fallbackValues[SettingKeyOpenAISchedulerObservabilityEnabled])
				schedulerObservabilityMaxTraces = parseOpenAISchedulerObservabilityMaxTraces(fallbackValues[SettingKeyOpenAISchedulerObservabilityMaxTraces])
				schedulerObservabilityRetentionDays = parseOpenAISchedulerObservabilityRetentionDays(fallbackValues[SettingKeyOpenAISchedulerObservabilityRetentionDays])
		...
		openAIAdvancedSchedulerSettingCache.Store(&cachedOpenAIAdvancedSchedulerSetting{
			...
			schedulerObservabilityEnabled:       schedulerObservabilityEnabled,
			schedulerObservabilityMaxTraces:     schedulerObservabilityMaxTraces,
			schedulerObservabilityRetentionDays: schedulerObservabilityRetentionDays,
...
func openAIAdvancedSchedulerRuntimeSettingKeys() []string {
	keys := []string{
		...
		SettingKeyOpenAISchedulerObservabilityEnabled,
		SettingKeyOpenAISchedulerObservabilityMaxTraces,
		SettingKeyOpenAISchedulerObservabilityRetentionDays,
...
// TTFT is useful for cold channels while real traffic and recent persisted
// history receive more weight once available.
func (s *OpenAIGatewayService) ReportChannelMonitorProbe(accountID int64, model string, success bool, firstTokenMs *int) {
	if s == nil || accountID <= 0 {
		return
	}
	s.openaiSchedulerOnce.Do(func() {
...
func (s *OpenAIGatewayService) RefreshOpenAISchedulerHealth(ctx context.Context) {
	...
	healthRepo, ok := s.usageLogRepo.(openAISchedulerHealthSnapshotRepository)
	if !ok {
		return
	}
	now := time.Now()
	lastNanos := s.openaiSchedulerHealthHydratedAt.Load()
	if lastNanos > 0 && now.Sub(time.Unix(0, lastNanos)) < openAISchedulerHealthRefreshInterval {
		return
	}
	// Reserve the refresh slot before querying so concurrent reconciliation and
	// request-start hydration cannot stampede the usage/error tables.
	if !s.openaiSchedulerHealthHydratedAt.CompareAndSwap(lastNanos, now.UnixNano()) {
		return
	}
	snapshots, err := healthRepo.GetOpenAISchedulerHealthSnapshots(ctx, now.Add(-openAISchedulerHealthHistoryWindow))
	if err != nil {
		s.openaiSchedulerHealthHydratedAt.Store(lastNanos)
		slog.Warn("openai_scheduler_health_refresh_failed", "error", err)
		return
	}
	s.openaiSchedulerOnce.Do(func() {
		if s.openaiAccountStats == nil {
...
// SelectPinnedAccountWithSchedulerForCapability revalidates and reacquires the
// exact account requested by a pool-mode same-account retry. It deliberately
// bypasses weighted competition; falling back to another account is the outer
// failover loop's responsibility after the retry budget is exhausted.
func (s *OpenAIGatewayService) SelectPinnedAccountWithSchedulerForCapability(
	ctx context.Context,
	accountID int64,
	groupID *int64,
	previousResponseID string,
	...
	ctx = context.WithValue(ctx, openAIPinnedRetryAccountContextKey{}, accountID)
	return s.SelectAccountWithSchedulerForCapability(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs,
		requiredTransport, requiredCapability, requireCompact, previousResponseCanMove, useUpstreamTokenCost, platformOverride...)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	pinnedAccountID, _ := ctx.Value(openAIPinnedRetryAccountContextKey{}).(int64)
...
func buildOpenAIAccountSchedulerScoreSnapshot(
	...
	for i := range candidates {
		...
		candidate.baseScore = weights.Priority*priorityFactor +
			weights.Load*loadFactor +
			weights.Queue*queueFactor +
			weights.ErrorRate*errorFactor +
			weights.TTFT*ttftFactor +
			...
			weights.QuotaHeadroom*quotaHeadroomFactor +
			weights.UpstreamCost*(upstreamCostFactor-openAIUpstreamCostNeutralFactor)
		if candidate.baseScore > bestBaseScore {
			bestBaseScore = candidate.baseScore
		}
	}
	...
	for _, candidate := range candidates {
		...
		if stickyWeightedEnabled {
			score.StickyScore = candidate.baseScore
			if openAIWeightedStickyWithinScoreGap(candidate.baseScore, bestBaseScore) {
				score.StickyScore += weights.Previous + weights.SessionSticky
			}
		}
