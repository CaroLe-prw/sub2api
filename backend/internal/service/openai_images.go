...
func (s *OpenAIGatewayService) ParseOpenAIImagesRequest(c *gin.Context, body []byte) (*OpenAIImagesRequest, error) {
	...
	effective := *req
	// Composite middleware may already have resolved the public alias to an
	// OpenAI-compatible upstream model. Preserve that resolution for forwarding.
	...
	if err := validateCompatibleImagesModel(effective.Model); err != nil {
		return nil, err
	}
