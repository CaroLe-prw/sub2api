...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	eventMarksUpstreamProgress := false
	...
	completeGuardedEvent := func(queueDrained bool) {
		...
		completedUpstreamProgressEvent := eventMarksUpstreamProgress
		...
		if completedUpstreamProgressEvent && !firstOutputProgressObserved {
			firstOutputProgressObserved = true
			stopFirstOutputTimer()
		}
		if completedProgressEvent && !firstOutputCommitted {
			firstOutputScanGuard.Store(false)
			firstOutputCommitted = true
		}
	...
	handleScanErr := func(scanErr error) (*openaiStreamingResult, error, bool) {
		...
		if errors.Is(scanErr, context.Canceled) || errors.Is(scanErr, context.DeadlineExceeded) {
			if eventShouldFlush {
				flushPending("Upstream canceled during stream flush, returning collected usage")
			}
			return resultWithUsage(), fmt.Errorf("stream usage incomplete: %w", scanErr), true
		}
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if guardFirstOutput {
				...
				eventMarksUpstreamProgress = eventMarksUpstreamProgress || openAIStreamDataMarksUpstreamProgress(data, eventType)
