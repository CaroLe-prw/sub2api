...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	if streamInterval == 0 && configuredStreamInterval > 0 && account != nil && account.Platform == PlatformOpenAI {
		// Keep a lightweight ticker for authoritative bare-error completion. A
		// silent, otherwise healthy OpenAI request still has no local read deadline.
		streamInterval = configuredStreamInterval
	}
	...
	eventStartsFirstOutputProgress := false
	...
	completeGuardedEvent := func(queueDrained bool) {
		...
		completedProgressEvent := eventStartsFirstOutputProgress
	...
	handleScanErr := func(scanErr error) (*openaiStreamingResult, error, bool) {
		...
		if errors.Is(scanErr, context.Canceled) || errors.Is(scanErr, context.DeadlineExceeded) {
			if eventShouldFlush {
				flushPending("Upstream canceled after semantic output, flushing residual response")
			}
			return resultWithUsage(), fmt.Errorf("stream usage incomplete: %w", scanErr), true
		}
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
					...
					if !cyberHit {
						if eventType == "error" {
							shouldFailover = openAIStreamErrorEventShouldFailover(dataBytes, failedMessage) ||
								(isOpenAIContextWindowError(failedMessage, dataBytes) && openAIStreamFailedEventShouldFailover(dataBytes, failedMessage, c))
						} else {
			...
			if stageFirstOutput {
				...
				eventStartsFirstOutputProgress = eventStartsFirstOutputProgress || startsFirstOutputProgress
	...
	for {
		...
		case <-intervalCh:
			...
			if account != nil && account.Platform == PlatformOpenAI && s.openAIStreamDataInterval(account) == 0 {
				continue
			}
			if clientDisconnected {
