...
func TestSettingService_UpdateSettings_PersistsChannelMonitorRequireAuth(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{ChannelMonitorRequireAuth: true})

	require.NoError(t, err)
	require.Equal(t, "true", repo.updates[SettingKeyChannelMonitorRequireAuth])
}
