...
var providerOpenAIResponsesAdapter = providerAdapter{
	...
	buildBody: func(model, prompt string) ([]byte, error) {
		return json.Marshal(map[string]any{
			...
			// 使用 Responses API 的规范消息数组，而不是字符串简写。部分兼容上游
			// 虽然能处理 Codex 的数组请求，却会在收到字符串 input 时返回 5xx。
...
func callProvider(
	...
	if opts != nil && opts.Streaming {
		return postStreamingJSON(ctx, full, body, headers, provider, apiMode)
	}
...
func postStreamingJSON(
	...
	text, rawBody, latency, err := readMonitorStreamingResponse(
		resp.Body, provider, apiMode, startedAt,
	)
	if err != nil {
