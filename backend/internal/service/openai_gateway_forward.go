...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.IsOpenAI() && (account.IsOpenAIApiKey() || account.IsOpenAIOAuthLike()) {
		...
		forceReasoningIDStrip := account.IsOpenAIApiKey() && openAIResponsesItemReferenceRecoveryRequested(c)
		sanitizedBody, changed, sanitizeErr := sanitizeOpenAIResponsesInputItemIDsWithOptions(body, forceReasoningIDStrip || openAIResponsesStoreDisabled(body))
		if sanitizeErr != nil {
	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			if openAIContextCompactionHTTPShouldFailover(c, resp.StatusCode, upstreamMsg, respBody) ||
				s.shouldFailoverOpenAIUpstreamResponse(account, resp.StatusCode, upstreamMsg, respBody) {
				upstreamDetail := ""
