...
// BatchSetGroupModelAllowlist applies one validated model allowlist to
// the selected groups without changing any other group fields.
func (s *adminServiceImpl) BatchSetGroupModelAllowlist(ctx context.Context, groupIDs []int64, config GroupModelAllowlist) (int, error) {
	...
	normalized, err := normalizeGroupModelAllowlist(config)
	if err != nil {
		return 0, err
	}
	config = normalized
	affected := 0
	if batchRepo, ok := s.groupRepo.(GroupModelAllowlistBatchRepository); ok {
		count, err := batchRepo.BatchUpdateModelAllowlist(ctx, uniqueIDs, config)
		if err != nil {
			return 0, err
		}
		affected = count
