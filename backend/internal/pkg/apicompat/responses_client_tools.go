...
func AdaptResponsesClientToolsWithInheritedMapping(
	...
	if rawTools, toolsPresent := req["tools"]; toolsPresent {
		if rawTools == nil {
			return ResponsesClientToolMapping{}, false, nil
		}
		if tools, ok := rawTools.([]any); ok && len(tools) == 0 {
			return ResponsesClientToolMapping{}, false, nil
		}
	...
	visit = func(value any) error {
		...
		case map[string]any:
			...
			case "custom_tool_call":
				// Continuation history must remain valid when this turn omits tools.
