...
func TestLockAndMergeAccountExtraPreservesLatestCodexTicket(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "current_extra", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot"}).
			AddRow(true, false, true, nil, nil, nil, nil, nil, nil, []byte(`{"codex_turn_ticket:model":{"state":"latest-database-ticket"},"newapi_sync_enabled":true,"newapi_user_access_token":"latest-encrypted-token","old_admin_setting":true}`), false, nil, nil))
	account := &service.Account{ID: 41, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth, Credentials: map[string]any{"access_token": "test"}, Extra: map[string]any{"codex_turn_ticket:model": map[string]any{"state": "stale"}, "codex_turn_ticket:injected": map[string]any{"state": "spoofed"}, "new_admin_setting": true, service.NewAPISyncEnabledExtraKey: false, service.NewAPIUserAccessTokenExtraKey: service.NewAPISecretMask}}
	extra, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil, nil)
	require.NoError(t, err)
	require.Equal(t, map[string]any{"state": "latest-database-ticket"}, extra["codex_turn_ticket:model"])
	require.Equal(t, true, extra[service.NewAPISyncEnabledExtraKey])
	require.Equal(t, "latest-encrypted-token", extra[service.NewAPIUserAccessTokenExtraKey])
