...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetMaxGroupRateMultiplier(key.MaxGroupRateMultiplier).
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldName,
			apikey.FieldStatus,
			apikey.FieldMaxGroupRateMultiplier,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.MaxGroupRateMultiplier {
		builder.SetMaxGroupRateMultiplier(key.MaxGroupRateMultiplier)
	}
	if fields.Quota {
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		Name:                   m.Name,
		Status:                 m.Status,
		MaxGroupRateMultiplier: m.MaxGroupRateMultiplier,
