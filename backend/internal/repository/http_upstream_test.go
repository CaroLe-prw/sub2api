...
func TestHTTPUpstreamRecordsActualErrorExchange(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		raw, err := io.ReadAll(r.Body)
		require.NoError(t, err)
		require.Contains(t, string(raw), "upstream-secret")
		w.Header().Set("Content-Type", "application/json")
		w.Header().Set("x-request-id", "rid-example")
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error":{"code":"invalid_type"},"api_key":"response-secret"}`))
	}))
	defer server.Close()
	inbound := service.EnableOpsRequestDiagnostics(httptest.NewRequest("POST", "/v1/responses", strings.NewReader(`{"input":"original"}`)))
	_, err := io.ReadAll(inbound.Body)
	require.NoError(t, err)
	request, err := http.NewRequestWithContext(inbound.Context(), "POST", server.URL, strings.NewReader(`{"input":"transformed","api_key":"upstream-secret"}`))
	require.NoError(t, err)
	response, err := NewHTTPUpstream(nil).Do(request, "", 42, 1)
	require.NoError(t, err)
	raw, err := io.ReadAll(response.Body)
	require.NoError(t, err)
	require.Contains(t, string(raw), "response-secret")
	require.NoError(t, response.Body.Close())
	diagnostic := service.BuildOpsRequestDiagnosticsJSON(inbound.Context())
	require.NotNil(t, diagnostic)
	require.Contains(t, *diagnostic, "original")
	require.Contains(t, *diagnostic, "transformed")
	require.Contains(t, *diagnostic, "invalid_type")
	require.Contains(t, *diagnostic, "rid-example")
	require.NotContains(t, *diagnostic, "upstream-secret")
	require.NotContains(t, *diagnostic, "response-secret")
}
