...
func TestLotterySettlementCreditsSingleEntrantWithThirdPrize(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT r.id, r.starts_at, r.ends_at,")).
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"first_name", "first_reward", "first_weight",
			"second_name", "second_reward", "second_weight",
			"third_name", "third_reward", "third_weight",
			"settled_at", "created_at", "participant_count",
		}).AddRow(
			17, start, end,
			service.LotteryFirstPrizeName, 1.0, 10,
			service.LotterySecondPrizeName, 0.5, 30,
			service.LotteryThirdPrizeName, 0.2, 60,
			nil, start, 1,
		))
