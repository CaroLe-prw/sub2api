...
func (r *groupRepository) BatchUpdateModelAllowlist(ctx context.Context, groupIDs []int64, config service.GroupModelAllowlist) (int, error) {
	if len(groupIDs) == 0 {
		return 0, nil
	}
	return r.client.Group.Update().
		Where(group.IDIn(groupIDs...)).
		SetModelAllowlist(service.DomainGroupModelAllowlist(config)).
		Save(ctx)
}
