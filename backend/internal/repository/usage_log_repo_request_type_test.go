...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$1.*GROUP BY GROUPING SETS").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"cost",
			"actual_cost",
			"account_cost", "user_account_cost", "probe_account_cost",
			"avg_duration_ms",
		}).
			AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 1.1, 0.1, 20.0).
			AddRow(0, 1, "/v1/responses", nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 1.1, 0.1, 20.0).
			AddRow(1, 0, nil, "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 1.1, 0.1, 20.0).
			AddRow(0, 0, "/v1/responses", "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 1.1, 0.1, 20.0))
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE \\(request_type = \\$1 OR \\(request_type = 0 AND stream = FALSE AND openai_ws_mode = FALSE\\)\\).*GROUP BY GROUPING SETS").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"account_cost", "user_account_cost", "probe_account_cost",
			"avg_duration_ms",
		}).AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 1.2, 0.0, 20.0))
	...
	require.Equal(t, 1.2, *stats.TotalUserAccountCost)
	require.NotNil(t, stats.TotalProbeAccountCost)
	require.Zero(t, *stats.TotalProbeAccountCost)
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped", "upstream_grouped", "inbound_endpoint", "upstream_endpoint",
			"requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
			"cost", "actual_cost", "account_cost", "user_account_cost", "probe_account_cost", "avg_duration_ms",
		}).AddRow(1, 1, nil, nil, int64(50), int64(1000), int64(2000), int64(60), int64(40), 15.0, 12.5, 11.0, 10.25, 0.75, 100.0))
	...
	require.NotNil(t, stats.TotalUserAccountCost)
	require.Equal(t, 10.25, *stats.TotalUserAccountCost)
	require.NotNil(t, stats.TotalProbeAccountCost)
	require.Equal(t, 0.75, *stats.TotalProbeAccountCost)
	require.NoError(t, mock.ExpectationsWereMet())
}
