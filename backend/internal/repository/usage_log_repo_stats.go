...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	query := fmt.Sprintf(`
		WITH scoped AS (
			SELECT
				...
				CASE WHEN request_type = %d THEN 0 ELSE COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1) END AS user_account_cost,
				CASE WHEN request_type = %d THEN COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1) ELSE 0 END AS probe_account_cost,
				duration_ms
			FROM usage_logs
			%s
		)
		SELECT
			...
			COALESCE(SUM(user_account_cost), 0) AS user_account_cost,
	...
	for rows.Next() {
		var (
			...
			cost, actualCost, accountCost, userAccountCost, probeAccountCost     float64
		...
		case inboundGrouped == 1 && upstreamGrouped == 1:
			...
			totalUserAccountCost = userAccountCost
			totalProbeAccountCost = probeAccountCost
