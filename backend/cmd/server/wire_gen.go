...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	vaptchaVerifier := repository.NewVaptchaVerifier()
	vaptchaService := service.NewVaptchaService(vaptchaVerifier)
	...
	openAISchedulerObservabilityRepository := repository.NewOpenAISchedulerObservabilityRepository(db)
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, openAISchedulerObservabilityRepository)
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, settingService, accountRepository, settingRepository)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, usageLogRepository, billingService, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, settingService, pluginManager)
	...
	schedulerObservabilityHandler := admin.NewSchedulerObservabilityHandler(openAIGatewayService)
	...
	subscriptionQuotaResetService := service.ProvideSubscriptionQuotaResetService(subscriptionService, userSubscriptionRepository, groupRepository, settingRepository, announcementService, leaderLockCache, db)
	...
	scheduledTestService := service.ProvideScheduledTestService(scheduledTestPlanRepository, scheduledTestResultRepository, usageLogRepository)
	scheduledTestHandler := admin.NewScheduledTestHandler(scheduledTestService, accountTestService, openAIGatewayService)
	...
	checkInService := service.NewCheckInService(checkInRepository, settingService)
	checkInHandler := admin.NewCheckInHandler(checkInService)
	lotteryRepository := repository.NewLotteryRepository(db)
	lotteryService := service.ProvideLotteryService(lotteryRepository, settingService)
	...
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db, secretEncryptor, configConfig, opsService)
	qqBotCache := repository.NewQQBotCache(redisClient)
	qqBotService := service.ProvideQQBotService(settingRepository, secretEncryptor, qqBotCache, settingService, channelMonitorService, channelMonitorV2Service)
	qqBotHandler := admin.NewQQBotHandler(qqBotService)
	adminHandlers := handler.ProvideAdminHandlers(qqBotHandler, dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, schedulerObservabilityHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, checkInHandler, lotteryHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, settingService)
	...
	scheduledTestRunnerService := service.ProvideScheduledTestRunnerService(scheduledTestPlanRepository, scheduledTestService, accountTestService, rateLimitService, configConfig, accountRepository, settingRepository, openAIGatewayService)
