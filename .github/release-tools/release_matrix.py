...
# Accept upstream's three-part versions and this fork's four-part releases.
...
def plan(args):
    ...
    if args.dry_run:
        ...
        if not tag.startswith('v') or not VERSION_RE.fullmatch(version):
            raise ValueError(f'publishing requires a v-prefixed release version tag (vX.Y.Z or vX.Y.Z.N), got {tag!r}')
