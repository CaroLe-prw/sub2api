export default {
    ...
    usage: {
      ...
      response: {
        incomingRequest: 'Original client request (redacted)', upstreamRequest: 'Request sent upstream (redacted)',
        requestHelp: 'Request bodies are redacted and formatted to compare input with forwarded changes. Headers and URL queries are not stored; credential fields and recognized credentials are masked. Requests follow the existing usage-record retention policy.',
        requestBody: 'View redacted request body',
        requestTruncated: 'The request was fully parsed and redacted; only the first {limit} is retained for display. A cut-off excerpt does not indicate a damaged request.',
        requestOmitted: { inspection_limit: 'Request exceeds the {limit} safe inspection limit; body not stored.', incomplete_body: 'Request body was not fully read and could not be safely redacted; body not stored.', invalid_json: 'Request is not complete valid JSON and could not be safely redacted; body not stored.', unsupported_body: 'Non-JSON or compressed request; only the observed byte count is recorded.' },
        ...
        title: 'Request / response records', view: 'View request / response', upstream: 'Upstream bytes read', downstream: 'Bytes sent to client',
        help: 'Alerts on text or another JSON document appended outside a complete JSON document, or extra data events after the end marker. Trailing symbols and recognized heartbeat markers are informational; normal SSE events are valid. This does not judge whether the model generated unwanted answer content.',
        unavailable: 'Not recorded', loadError: 'Failed to load response',
        partial: 'An unfinished read or failed write left the capture incomplete. The full response format cannot be assessed.',
        truncatedHelp: 'Capture truncated: only the first {limit} per side is retained. The last JSON may be cut off by this limit; this does not establish a damaged response. Earlier format errors are reported separately.',
        raw: 'View raw JSON / SSE', firstJSON: 'First JSON (up to 4 KiB)', extraSample: 'Trailing characters / event excerpt (up to 4 KiB)',
        status: { tail_symbols: 'Trailing symbols only', ok: 'No additional content found', extra: 'Legacy format notice; review needed', extra_content: 'Additional text / JSON', truncated: 'Capture truncated', incomplete: 'Incomplete capture', invalid: 'Invalid JSON', non_json: 'Non-JSON content', unavailable: 'Not recorded' },
        issue: { trailing_symbols: 'Event {frame}: framing symbols or heartbeat marker only; no extra-content alert', trailing_content: 'Event {frame}: additional text or JSON outside the JSON document', invalid_json: 'Event {frame}: invalid JSON', after_done: 'Event {frame}: data event after [DONE]', capture_truncated: 'Event {frame}: capture cut off here; full JSON cannot be checked', incomplete_capture: 'Event {frame}: JSON not fully captured; format undetermined' }
      },
