export default {
    ...
    riskControl: {
      ...
      configHint: 'Use a /v1/moderations-compatible API to score request content and handle threshold hits by mode.',
      ...
      baseUrl: 'Moderation API Base URL',
      model: 'Model',
      ...
      riskThresholdsHint: 'Adjust hit thresholds for categories returned by the selected moderation model. Scores greater than or equal to the threshold count as hits.',
    ...
    channelMonitor: {
      ...
      autoModels: {
        ...
        mode: 'Probe mode',
        modeFixed: 'Fixed mode',
        modeFixedHint: 'Probe each account representative and any explicitly listed account models at the configured interval.',
        modeAdaptive: 'Adaptive mode',
        modeAdaptiveHint: 'Confirm cold accounts every 10 minutes, slow down after 3 successes, skip after recent real traffic, and back failures off through 5/10/20/40/60 minutes.',
        fixedInterval: 'Fixed probe interval',
        fixedIntervalHint: 'Only applies to fixed mode. Set from 1 to 1440 minutes; the default is 5 minutes.',
        minutes: 'minutes',
      ...
      dataPanel: {
        ...
        loadError: 'Failed to load account-pool probe data',
        ...
        heartbeatLegend: 'Card heartbeats use 5-minute buckets: green = all healthy, orange = slow first token or partial degradation, red = all failed, gray = incomplete with no known failure.',
        aggregateStatus: { success: 'All healthy', degraded: 'Partially degraded', failed: 'All failed', partial: 'Incomplete samples' },
        aggregateBreakdown: '{healthy} healthy · {slow} slow first token · {failed} failed',
        modelCoverage: '{observed}/{expected} models covered',
        channelCoverage: '{observed}/{expected} channels covered',
        slowestFirstToken: 'Slowest first token',
        slowestTotalDuration: 'Slowest total duration',
        historyScope: 'The heartbeat bar shows the latest 60 samples (~5h on schedule). Each model keeps up to 288 samples (~24h); manual probes also count toward retention.',
        visibleSampleCount: '{n}/{limit} samples',
        timelineStart: 'Start {time}',
        timelineLast: 'Last {time}',
        relativeTime: { now: 'now', minutes: '{n}m ago', hours: '{n}h ago', days: '{n}d ago' },
        firstToken: 'First token',
        totalDuration: 'Total duration',
        unavailable: 'No data',
        streaming: 'Streaming probe by default',
        combinedHealth: 'Real traffic + active probes',
        ...
        sourceUser: 'Real user call',
        sourceProbe: 'Active probe',
        activeProbeSummary: 'Active probes (loaded)',
        resultBreakdown: '{success} succeeded · {failed} failed',
        probeBreakdown: '{availability} available · {failed} failed · {latency} avg latency',
        modelSourceCounts: 'User 30m {users} · Loaded probes {probes}',
        combinedStatus: { success: 'Combined healthy', degraded: 'Available with issues', failed: 'Combined failed', pending: 'No samples' },
        ...
        detectedModels: 'Observed models',
        concurrency: 'Concurrency',
        avgLatency: 'Average latency',
        availability: 'Availability',
        validSamples: '{n} valid samples',
        totalSamples: '{n} probe samples',
        abnormalSamples: 'Failed samples (red)',
        retainedSamples: 'Counts only the {n} loaded records',
        ...
        probeStatus: { success: 'Online', degraded: 'Slow response', failed: 'Failed', pending: 'Pending' },
    ...
    subscriptions: {
      ...
      resetQuotaConfirm: "Choose the quota periods and window-time strategy to reset for '{user}'.",
      selectAtLeastOneQuota: 'Select at least one of daily, weekly, or monthly quota',
      ...
      guide: {
        ...
        actions: {
          ...
          resetQuota: 'Reset Quota',
          resetQuotaDesc: 'Choose any daily/weekly/monthly quotas and window-time strategy',
          revoke: 'Revoke',
    ...
    quotaReset: {
      title: 'Quota Reset',
      description: 'Reset subscription usage manually or on a schedule by subscription group, independently of announcements',
      statusTitle: 'Runtime Status',
      statusDescription: 'View the latest reset result and next scheduled run',
      refresh: 'Refresh Status',
      refreshFailed: 'Failed to refresh runtime status',
      runNow: 'Run Now',
      running: 'Running...',
      runConfirmTitle: 'Run Quota Reset Now',
      manualDescription: 'These selections apply to this run only and do not save or change the schedule below. This run can optionally move the next scheduled time.',
      manualSummaryTitle: 'Confirm This Run',
      manualNoGroups: 'None selected',
      manualAnnouncementHint: 'If at least one subscription is reset, a popup announcement will be sent to the groups selected here.',
      manualScheduleMode: 'Following Schedule',
      manualRestartSchedule: 'Delay the Next Run (Recommended)',
      manualRestartScheduleHint: 'If at least one subscription is reset, calculate the next run from this run\'s completion time',
      manualKeepSchedule: 'Keep the Existing Schedule',
      manualKeepScheduleHint: 'Do not change the next run; another reset may happen soon if it is already close',
      runCompleted: 'Quota reset completed',
      runFailed: 'Failed to run quota reset',
      configTitle: 'Reset Configuration',
      configDescription: 'Configure the schedule, target subscription groups, quota periods, and window strategy independently',
      enabled: 'Enable Automatic Reset',
      enabledHint: 'Run automatically at the interval below; Run Now remains available while disabled',
      intervalHours: 'Interval (Hours)',
      intervalHint: 'Hours between automatic runs; must be a positive integer',
      groupsHint: 'Only subscription-billing groups are shown; multiple groups can be selected',
      save: 'Save Configuration',
      saving: 'Saving...',
      configSaved: 'Quota reset configuration saved',
      saveFailed: 'Failed to save quota reset configuration',
      loadFailed: 'Failed to load quota reset configuration',
      invalidInterval: 'Interval must be a positive whole number of hours',
      selectGroup: 'Select at least one subscription group',
      selectWindow: 'Select at least one of daily, weekly, or monthly quota',
      ...
      policy: {
        windows: 'Quota Periods to Reset',
        daily: 'Daily',
        weekly: 'Weekly',
        monthly: 'Monthly',
        windowMode: 'Window-Time Strategy',
        modes: {
          current: 'Start From Current Time',
          currentHint: 'Clear usage and set each selected window start to the execution time',
          naturalDay: 'Start From Today at 00:00',
          naturalDayHint: 'Clear usage and set window starts to 00:00 today in the server timezone; daily, weekly, and monthly windows still roll for 24 hours, 7 days, and 30 days',
          preserve: 'Preserve Current Window',
          preserveHint: 'Only clear selected usage without changing existing window starts; if a window has already expired, the next request may reset it again immediately'
        }
      }
    },
