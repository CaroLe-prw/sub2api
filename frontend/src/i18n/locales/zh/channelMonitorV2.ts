...
export default {
  channelMonitorV2: {
    ...
    matrix: {
      title: '可用性趋势', description: '每行是一种渠道组合，每个色块代表一个统计区间；真实调用不足时自动合并账户探测', wheelZoom: '在色块上滚轮放大（区间变窄、色块变宽）', wheelZoomX: '在色块上滚轮放大（区间变窄、色块变宽）', dimension: '渠道维度', emptyTitle: '当前筛选窗口没有矩阵数据', legendAria: '健康分数图例', bad: '差', good: '好', healthyLegend: '健康 (≥80)', warningLegend: '需关注 (50–79)', criticalLegend: '异常 (<50)', unknownLegend: '无调用且无账户探测 / 样本不足', noTraffic: '该区间无可用性信号', noTrafficAt: '{time} · 无可用性信号', scoreLine: '健康分 {score}', resetZoom: '重置缩放', availabilityEvidence: '可用性依据：{source}', availabilitySources: { probe: '账户探测（无用户流量）', mixed: '真实调用 + 账户探测' }
    },
    chart: {
