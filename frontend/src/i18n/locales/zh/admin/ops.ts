export default {
    ops: {
      ...
      telegram: {
        title: 'Telegram 通知模板',
        description: '配置可复用的 Bot、群聊和话题投递目标。',
        enabled: '启用模板',
        templateName: '模板名称',
        templateNamePlaceholder: '例如：运维告警群',
        templatesTitle: 'Telegram 模板',
        templatesDescription: '每个模板可以使用不同 Bot、群聊和论坛话题，并可被多个通知类型复用。',
        addTemplate: '添加模板',
        noTemplates: '暂无 Telegram 模板。',
        unnamedTemplate: '未命名模板',
        routesTitle: '通知路由',
        routesDescription: '为不同通知类型选择投递模板。',
        opsAlertTemplate: '运维告警模板',
        noTemplate: '不发送 Telegram 通知',
        manageInSystemSettings: '模板内容请在“系统设置 > 通知设置”中维护。',
        upstreamRateChange: '上游倍率变动通知',
        upstreamRateChangeHint: '当 Sub2API 探测或 NewAPI 同步发现账号倍率发生变化时发送通知；首次探测不发送。',
        upstreamRateChangeTemplate: '倍率变动通知模板',
        upstreamBalanceLow: '上游低余额通知',
        upstreamBalanceLowHint: '账号探测到上游余额首次低于账号阈值时发送；余额未恢复期间不会重复通知。',
        upstreamBalanceLowTemplate: '低余额通知模板',
        botToken: 'Bot Token',
        ...
        loadFailed: 'Telegram 通知设置加载失败',
        saveSuccess: 'Telegram 通知设置保存成功',
        saveFailed: 'Telegram 通知设置保存失败',
        validation: {
          nameRequired: '请填写 Telegram 模板名称',
          nameTooLong: 'Telegram 模板名称不能超过 100 个字符',
          tooManyTemplates: 'Telegram 模板最多只能配置 50 个',
          ...
          rateChangeTemplateRequired: '启用上游倍率变动通知时必须选择模板',
          balanceLowTemplateRequired: '启用上游低余额通知时必须选择模板'
        }
      },
      settings: {
        title: '运维监控设置',
