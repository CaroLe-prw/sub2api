export default {
    accounts: {
      ...
      newapiSync: {
        title: 'NewAPI 上游分组倍率同步',
        description: '使用账号自身的端点和 API Key，按 NewAPI 用户有效分组配置同步计费倍率；定时任务遵循全局上游倍率探测开关和间隔。',
        enabled: '启用定时同步',
        ...
        testSuccess: 'NewAPI 连接与倍率解析测试通过，未修改账号倍率',
        testFailed: 'NewAPI 连接测试失败',
        syncChanged: 'NewAPI 倍率已同步并更新账号倍率',
        syncUnchanged: '同步完成，账号倍率无需更新',
        syncFailed: 'NewAPI 倍率同步失败，原倍率保持不变',
        statuses: {
          never: '尚未同步',
          ok: '同步成功',
          failed: '同步失败'
