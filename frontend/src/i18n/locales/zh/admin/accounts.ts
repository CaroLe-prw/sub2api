export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        schedulingSourceCalibrated: '来源：上游声明 × 校准系数',
        schedulingSourceFallback: '来源：账号计费倍率兜底',
        ...
        newapiGroupRate: 'NewAPI 实际分组 {group}：{value}x',
        userRate: '用户专属倍率：{value}x',
        peakRate: '高峰：{start}-{end}，{value}x（{timezone}）',
        noPeakRate: '高峰倍率：未启用',
        effectiveRate: '当前倍率：{value}x',
        walletBalance: '上游钱包余额：{value}',
        subscriptionRemaining: '上游套餐剩余额度：{value}',
        balanceUnavailable: '余额不足',
        balanceLow: '余额不足：当前 {value}，提醒阈值 {threshold}',
        balanceAlert: '上游低余额提醒',
        balanceAlertHint: '全局默认开启，提醒阈值为 $5；账号可单独覆盖或关闭，余额恢复后再次跌破会重新发送 Telegram 提醒。',
        balanceAlertThreshold: '提醒阈值',
        balanceAlertThresholdHint: '按上游返回的美元余额判断，不受账号计费倍率或调度校准系数影响。',
        ...
        enabled: '打开',
        disabled: '关闭',
        probeFailed: '探测上游计费信息失败',
        noEligibleAccounts: '请选择 API Key 账号',
        batchLimit: '每次最多探测 20 个账号',
        batchCompleted: '已完成 {count} 个账号的上游计费信息探测',
        batchPartial: '上游计费信息探测部分完成：成功 {success} 个，失败 {failed} 个'
      },
      newapiSync: {
        title: 'NewAPI 上游同步',
        description: '同步 NewAPI 用户分组倍率与原始余额额度；定时任务遵循全局上游探测开关和间隔。',
        enabled: '启用定时同步',
        baseUrl: 'NewAPI Base URL（可选）',
        baseUrlHint: '留空时使用该账号的端点地址；显式填写时仅允许符合系统出站安全策略的 HTTP/HTTPS 地址。',
        userId: 'NewAPI UID',
        accessToken: '用户访问令牌',
        apiKey: 'API Key',
        apiKeyMissing: '账号尚未配置 API Key',
        apiKeyHint: '复用该账号的模型 API Key；这里只显示脱敏状态。',
        syncInterval: '同步间隔（分钟）',
        syncIntervalHint: '复用系统设置中的全局上游探测间隔。',
        secretPlaceholder: '输入新凭据，或保留掩码',
        secretHint: '凭据加密保存；留空或保留 ******** 不会覆盖现有值。',
        secretRequiredHint: '创建账号时启用 NewAPI 必填；账号创建后会加密保存该凭据。',
        createConfigRequired: '创建账号前请填写有效的 NewAPI UID 和用户访问令牌。',
        initialSetupFailed: '账号已创建，但 NewAPI 初始配置或同步失败；请在编辑弹窗中检查配置并重试。',
        testConnection: '测试连接',
        syncNow: '立即同步',
        status: '最近状态',
        lastSyncAt: '最近同步',
        userGroup: '用户分组',
        tokenGroup: 'API Key 配置分组',
        actualGroup: '实际分组',
        currentRatio: '校准后账号倍率',
        ratioSource: '倍率来源',
        crossGroupRetry: '跨分组重试',
        loadFailed: '加载 NewAPI 同步配置失败',
        saved: 'NewAPI 同步配置已保存',
        saveFailed: '保存 NewAPI 同步配置失败',
        testSuccess: 'NewAPI 连接、余额与倍率解析测试通过，未写入任何数据',
        testFailed: 'NewAPI 连接测试失败',
        syncChanged: 'NewAPI 余额已同步，账号倍率也已更新',
        syncUnchanged: 'NewAPI 余额同步完成，账号倍率无需更新',
        syncFailed: 'NewAPI 同步失败，原倍率和上一份有效余额保持不变',
        balance: {
          title: '上游余额',
          tokenName: 'API Key 名称',
          tokenRemaining: 'API Key 剩余额度',
          tokenUsed: 'API Key 已用额度',
          tokenTotal: 'API Key 总额度',
          tokenUnlimited: 'API Key 无限额度',
          accountRemaining: '账户剩余额度',
          accountUsed: '账户已用额度',
          accountTotal: '账户总额度',
          availability: '整体可用性',
          available: '有余额',
        ...
        errors: {
          ...
          NEWAPI_USER_ID_INVALID: 'UID 必须是正整数。',
          newapi_base_url_invalid: 'NewAPI Base URL 或账号端点地址不符合系统出站安全策略。',
          newapi_client_unavailable: 'NewAPI HTTP 客户端不可用。',
          newapi_configuration_incomplete: '请填写 UID、用户访问令牌，并确保账号自身已配置模型 API Key。',
          newapi_user_id_mismatch: '访问令牌所属 UID 与配置不一致。',
          newapi_token_user_mismatch: '模型 API Key 不属于配置的 UID。',
          newapi_token_not_enabled: '模型 API Key 当前不可用。',
          newapi_token_search_not_unique: '模型 API Key 搜索结果不是唯一一条。',
          newapi_auto_group_unsupported: 'API Key 分组为 auto，无法通过静态分组配置确定倍率；请改为固定分组后重试。',
          newapi_effective_group_empty: '无法解析实际使用分组。',
          newapi_effective_group_not_found: '用户有效分组列表中不存在该分组。',
          newapi_effective_ratio_invalid: '分组倍率缺失或不是有效的正数。',
          newapi_calibrated_ratio_invalid: '校准后的账号倍率不是有效的非负数。',
          newapi_user_self_invalid_response: '用户信息响应格式无效。',
          newapi_account_remaining_quota_invalid: '账户剩余额度不是有效的安全整数。',
          newapi_account_used_quota_invalid: '账户已用额度不是有效的非负安全整数。',
          newapi_account_total_quota_invalid: '账户总额度超出安全整数范围。',
          newapi_token_usage_invalid_response: 'API Key 余额响应格式无效。',
          newapi_token_remaining_quota_invalid: 'API Key 剩余额度不是有效的安全整数；有限额度 Key 不允许负数。',
          newapi_token_used_quota_invalid: 'API Key 已用额度不是有效的非负安全整数。',
          newapi_token_total_quota_invalid: 'API Key 总额度不是有效的非负安全整数。',
          newapi_token_expires_at_invalid: 'API Key 过期时间不是有效的非负安全整数。',
          newapi_token_search_invalid_response: '令牌搜索响应格式无效。',
          newapi_user_groups_invalid_response: '用户分组响应格式无效。',
          newapi_request_timeout: '请求 NewAPI 超时。',
          newapi_request_failed: '请求 NewAPI 失败。',
          newapi_http_error: 'NewAPI 接口返回了非成功状态。',
          newapi_request_build_failed: '无法创建 NewAPI 请求。',
          newapi_empty_response: 'NewAPI 返回了空响应。',
          newapi_response_read_failed: '读取 NewAPI 响应失败。',
          newapi_response_too_large: 'NewAPI 响应超过大小限制。',
          newapi_credential_decrypt_failed: '已保存的凭据无法解密，请重新填写。',
          newapi_sync_failed: 'NewAPI 倍率同步失败。',
          newapi_sync_unavailable: 'NewAPI 同步服务当前不可用。',
          newapi_identity_changed: '同步配置已发生变化，请重试。',
          newapi_sync_busy: '该账号正在同步，请稍后重试。'
        }
      },
      ...
      quotaLimitHint: '设置日/周/总上游容量，任一维度达到限额后账号暂停调度。容量口径与账号成本、调度成本相互独立。修改限额或额度倍率不会重置已用额度。',
      quotaLimitToggle: '启用配额限制',
      quotaLimitToggleHint: '开启后，当账号用量达到设定额度时自动暂停调度',
      quotaUsageMultiplier: '额度消耗倍率',
      quotaUsageMultiplierHint: '上游 token 费用按 1:1 消耗额度时填 1。此值只影响账号额度累计，不影响成本统计或调度成本；修改后请重置配额，避免新旧口径混合。',
      quotaDailyLimit: '日限额',
      ...
      openai: {
        ...
        forceFastMode: '强制上游 Fast 模式',
        forceFastModeDesc: '开启后，此账号转发到上游的请求固定使用 Fast（priority）；用户计费仍按原始请求的 Fast/Flex 模式计算。',
      ...
      groupPriority: '分组优先级',
      groupPriorityHint: '仅影响该分组内的调度，数值越小越优先',
      customizeGroupPriority: '自定义分组优先级',
      followAccountPriorityShort: '跟随 {priority}',
      allGroupsFollowAccountPriority: '所有已选分组均跟随账号优先级 {priority}',
      billingRateMultiplier: '账号计费倍率',
      billingRateMultiplierHint: '实际账号成本倍率；OpenAI 上游声明不可用时也作为调度兜底。0 表示不计费。',
      expiresAt: '过期时间',
      ...
      tokenAudit: {
        ...
        title: 'Token 用量验真',
        ready: '未运行',
        unsupported: '当前仅支持使用 Responses API 的 OpenAI API Key 账号。',
        model: '模型',
        run: '开始验真',
        running: '验真中...',
        cancel: '取消验真',
        cancelled: '已取消（保留已完成结果）',
        progress: '检测进度',
        preparing: '准备中...',
        retry: '重试此样本',
        action: '操作',
        failed: 'Token 验真失败',
        intercept: '固定输入',
        fixedContextLength: '固定上下文长度',
        slope: '可变斜率',
        completed: '已完成',
        tokenizer: 'Tokenizer',
        variableGrowth: '比例增长',
        fixedContextStatus: '固定项状态',
        outputCap: '输出上限',
        englishFit: '英文拟合',
        sample: '样本',
        local: '本地 Token',
