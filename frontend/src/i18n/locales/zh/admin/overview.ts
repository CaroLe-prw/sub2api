export default {
    ...
    dashboard: {
      ...
      accountCost: '成本',
      userRequests: '用户请求',
      automaticProbes: '自动探测',
      combinedTotal: '合计',
      userTokens: '用户 Token',
      probeTokens: '探测 Token',
      userRevenue: '用户消费',
      userRequestCost: '用户请求成本',
      probeCost: '探测成本',
      totalAccountCost: '总账号成本',
      standardListCost: '标准价格',
    ...
    groups: {
      ...
      modelMapping: {
        title: '分组模型映射',
        hint: '适用于所有平台及混合分组。在选号前将请求模型映射为目标模型，并按目标模型计价。例如 luna → terra，账户无需配置 luna，只需支持 terra。分组规则优先于渠道映射；账号可继续将目标模型映射为上游名称。精确匹配，删除全部规则即可关闭。',
        from: '请求模型，例如 luna',
        to: '转发及计价模型，例如 terra',
        add: '添加模型映射',
        invalid: '请填写完整、唯一的模型映射，最多 64 条，每个名称最多 200 字符，不支持通配符或控制字符。',
      },
      modelRouting: {
        title: '模型路由配置',
        tooltip:
