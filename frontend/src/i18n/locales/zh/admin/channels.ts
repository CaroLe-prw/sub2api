export default {
    ...
    riskControl: {
      ...
      refresh: '刷新',
      config: '内容审计配置',
      configHint: '调用兼容 /v1/moderations 的审核 API 进行内容评分，命中阈值后按模式处理。',
      openSettings: '内容审计设置',
      ...
      riskThresholds: '风险阈值',
      riskThresholdsHint: '按当前审核模型返回的分类调整命中阈值，分数达到或超过阈值即视为命中。',
    ...
    channelMonitor: {
      ...
      autoModels: {
        title: '号池模型自动监测',
        description: '自动汇总已启用、可调度的非 OAuth 上游账号模型映射，并按协议族选择低成本代表文本模型建立流式主动探针；不会改写手工监控。',
        enabled: '自动监测号池模型',
        enabledHint: '默认开启。仅纳入 active、可调度、非 OAuth 账号；新增账号、状态或模型策略会在一分钟内同步。',
        mode: '探测模式',
        modeFixed: '固定模式',
        modeFixedHint: '按设定间隔探测每个账号的代表模型，以及账号白名单中明确指定的模型。',
        modeAdaptive: '自适应模式',
        modeAdaptiveHint: '冷账号每 10 分钟确认，连续成功 3 次后降为 60 分钟；近期有真实成功流量会跳过，失败按 5/10/20/40/60 分钟退避。',
        fixedInterval: '固定探测间隔',
        fixedIntervalHint: '仅固定模式生效，范围 1 到 1440 分钟。默认 5 分钟。',
        minutes: '分钟',
        whitelist: '全局自动模型白名单',
        whitelistPlaceholder: 'gpt-5.*\nclaude-sonnet-4-*\ngemini-3-*',
        whitelistHint: '每行或逗号分隔；支持精确模型和结尾 * 通配符。全局白名单只限定候选范围，系统仍按协议族选择代表模型；不影响手工模型。',
        noModels: '暂无符合条件的模型',
        billingHint: '主动探测会产生真实上游请求。默认从账号模型映射中为每个协议族选择 1 个低成本文本模型；账号白名单中明确填写的精确模型会分别探测。',
        saved: '号池自动模型策略已保存',
      ...
      dataPanel: {
        ...
        noHistory: '等待首次探测',
        recentTrend: '近期探测趋势',
        heartbeatLegend: '卡片心跳按 5 分钟周期聚合：绿＝全部正常，橙＝存在慢首 T 或部分异常，红＝全部异常，灰＝样本未收齐且未发现异常。',
        aggregateStatus: { success: '全部正常', degraded: '部分异常', failed: '全部异常', partial: '样本未收齐' },
        aggregateBreakdown: '正常 {healthy} · 首 T 慢 {slow} · 异常 {failed}',
        modelCoverage: '覆盖 {observed}/{expected} 个模型',
        channelCoverage: '覆盖 {observed}/{expected} 个渠道',
        slowestFirstToken: '最慢首 T',
        slowestTotalDuration: '最慢总时长',
        historyScope: '心跳栏显示最新 60 条（自动探针约 5h）；每模型最多保留 288 条（约 24h），手动探测也计入保留条数。',
        visibleSampleCount: '{n}/{limit} 条',
        timelineStart: '起 {time}',
        timelineLast: '上次 {time}',
        relativeTime: { now: '刚刚', minutes: '{n}m前', hours: '{n}h前', days: '{n}d前' },
        firstToken: '首 T',
        totalDuration: '总时长',
        unavailable: '暂无数据',
        streaming: '默认流式探针',
        combinedHealth: '真实调用 + 主动探测',
        combinedState: '综合状态',
        ...
        sourceUser: '真实用户调用',
        sourceProbe: '主动探测',
        activeProbeSummary: '主动探测（已加载）',
        resultBreakdown: '{success} 成功 · {failed} 失败',
        probeBreakdown: '可用率 {availability} · {failed} 失败 · 均延迟 {latency}',
        modelSourceCounts: '用户30m {users} · 已载探测 {probes}',
        combinedStatus: { success: '综合正常', degraded: '可用但有异常', failed: '综合异常', pending: '暂无样本' },
        probeState: '探测状态',
        healthy: '探测正常',
        hasIssues: '存在异常',
        scheduling: '调度',
        ...
        avgLatency: '平均延迟',
        availability: '可用率',
        validSamples: '{n} 个有效样本',
        totalSamples: '{n} 个探测样本',
        abnormalSamples: '失败样本（红色）',
        retainedSamples: '仅统计已加载的 {n} 条记录',
        ...
        probeStatus: { success: '在线', degraded: '响应偏慢', failed: '异常', pending: '待探测' },
        columns: { channel: '渠道', model: '模型', status: '状态', latency: '延迟', availability: '7 天可用率', checkedAt: '最近探测', action: '操作' },
        accountWhitelist: {
          title: '渠道模型白名单',
          titleWithName: '渠道模型白名单 · {name}',
          description: '渠道白名单与全局白名单取交集。精确填写的模型会分别探测，通配符只限定候选范围并继续自动选择代表模型；不会影响手工监控。保存后一分钟内同步。',
    ...
    subscriptions: {
      ...
      resetQuotaConfirm: "请选择要为 '{user}' 重置的配额周期和窗口时间策略。",
      selectAtLeastOneQuota: '请至少选择日限、周限或月限中的一项',
      ...
      guide: {
        ...
        actions: {
          ...
          resetQuota: '重置配额',
          resetQuotaDesc: '自由选择日/周/月配额及窗口时间策略',
          revoke: '撤销',
    ...
    quotaReset: {
      title: '套餐重置',
      description: '按订阅分组自动或手动重置套餐用量，不依赖公告分组',
      statusTitle: '运行状态',
      statusDescription: '显示最近一次重置结果和下次计划执行时间',
      refresh: '刷新状态',
      refreshFailed: '刷新运行状态失败',
      runNow: '立即执行',
      running: '执行中...',
      runConfirmTitle: '确认立即执行',
      manualDescription: '本次选择只执行一次，不会保存或修改下方定时配置；是否调整下次定时执行时间由本次选项决定。',
      manualSummaryTitle: '本次执行确认',
      manualNoGroups: '尚未选择',
      manualAnnouncementHint: '只要至少一个订阅重置成功，就会按本次选择的分组发送一条弹窗公告。',
      manualScheduleMode: '后续定时计划',
      manualRestartSchedule: '顺延下一次定时任务（推荐）',
      manualRestartScheduleHint: '若本次至少成功重置一个套餐，则从本次完成时间重新计算下一次执行时间',
      manualKeepSchedule: '保持原定时计划',
      manualKeepScheduleHint: '不调整下一次执行时间；若原计划临近，本次执行后可能很快再次重置',
      runCompleted: '套餐重置执行完成',
      runFailed: '执行套餐重置失败',
      configTitle: '重置配置',
      configDescription: '独立配置计划任务、目标订阅分组、重置周期和窗口策略',
      enabled: '启用自动重置',
      enabledHint: '启用后按下方时间间隔自动执行；关闭时仍可立即执行',
      intervalHours: '执行间隔（小时）',
      intervalHint: '自动任务两次执行之间的小时数，必须为正整数',
      groupsHint: '这里只显示订阅计费类型的分组，可自由多选',
      save: '保存配置',
      saving: '保存中...',
      configSaved: '套餐重置配置已保存',
      saveFailed: '保存套餐重置配置失败',
      loadFailed: '加载套餐重置配置失败',
      invalidInterval: '执行间隔必须是大于 0 的整数小时',
      selectGroup: '请至少选择一个订阅分组',
      selectWindow: '请至少选择日限、周限或月限中的一项',
      matchedCount: '匹配订阅',
      resetCount: '重置成功',
      skippedCount: '跳过',
      failedCount: '失败',
      ...
      policy: {
        ...
        modes: {
          current: '从当前时间开始',
          currentHint: '清零用量，并将所选窗口的起点改为执行时刻',
          naturalDay: '按当天 00:00 开始',
          naturalDayHint: '清零用量，并将窗口起点设为服务端时区当天 00:00；日/周/月仍按 24 小时/7 天/30 天滚动',
          preserve: '保持当前窗口时间',
          preserveHint: '只清零所选用量，不修改现有窗口起点；若原窗口已到期，下次请求可能立即自动重置'
        }
      }
    },
