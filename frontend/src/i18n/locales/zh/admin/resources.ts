export default {
    ...
    usage: {
      ...
      requestId: '请求ID',
      response: {
        incomingRequest: '用户原始请求（脱敏）', upstreamRequest: '发给上游的请求（脱敏）',
        requestHelp: '请求体经脱敏并格式化后展示，用于对照输入和转发改写。请求头和 URL 查询参数不保存；按字段和已识别凭据屏蔽密钥。请求记录随使用记录的现有保留策略清理。',
        requestBody: '查看脱敏请求体',
        requestTruncated: '请求已完整解析和脱敏，仅保留前 {limit} 展示内容；末尾截断不代表实际请求损坏。',
        requestOmitted: { inspection_limit: '请求超过 {limit} 安全解析上限，未保存请求体。', incomplete_body: '请求体未完整读取，无法安全脱敏，未保存请求体。', invalid_json: '请求不是完整有效的 JSON，无法安全脱敏，未保存请求体。', unsupported_body: '此请求为非 JSON 或压缩内容，仅记录读取字节数。' },
        controlsEscaped: '原文含 NUL 控制字符，查看时以转义文本展示；检查结果基于原始内容。',
        upstreamShort: '上游', downstreamShort: '下发',
        title: '请求 / 响应记录', view: '查看请求 / 响应', upstream: '上游读取内容', downstream: '实际下发内容',
        help: '仅对完整 JSON 外追加的文本、另一段 JSON 或结束标记后的额外数据事件告警。单独尾随符号和已识别的心跳标记仅作普通提示，正常 SSE 多事件不告警。此检测不判断模型回答是否多生成了内容。',
        unavailable: '未记录', loadError: '响应内容加载失败',
        partial: '读取未结束或发送失败，记录不完整，无法判断完整响应的格式。',
        truncatedHelp: '记录截断：每侧仅保留前 {limit}。末尾 JSON 可能因此不完整，不代表实际响应损坏；截断之前的格式异常仍会单独标出。',
        raw: '查看原始 JSON / SSE', firstJSON: '首个 JSON（最多展示 4 KiB）', extraSample: '尾随字符 / 事件片段（最多展示 4 KiB）',
        status: { tail_symbols: '仅尾随符号', ok: '未发现额外内容', extra: '历史格式提示，待复核', extra_content: '有额外文本 / JSON', truncated: '记录截断', incomplete: '记录不完整', invalid: 'JSON 格式异常', non_json: '非 JSON 内容', unavailable: '未记录' },
        issue: { trailing_symbols: '第 {frame} 个事件：仅尾随符号或心跳标记，不作为额外内容告警', trailing_content: '第 {frame} 个事件：JSON 外有额外文本或 JSON', invalid_json: '第 {frame} 个事件：JSON 格式异常', after_done: '第 {frame} 个事件：[DONE] 后仍有数据事件', capture_truncated: '第 {frame} 个事件：记录在此截断，无法检查完整 JSON', incomplete_capture: '第 {frame} 个事件：未记录完整 JSON，暂无法判断格式' }
      },
