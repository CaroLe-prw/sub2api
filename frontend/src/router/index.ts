...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresSubscription || to.meta.requiresRiskControl || to.meta.requiresCheckIn || to.meta.requiresLottery) && !appStore.publicSettingsLoaded) {
    try {
      await appStore.fetchPublicSettings()
    } catch (error) {
      console.warn('Failed to load public settings in route guard', error)
