...
describe('admin AccountsView lite account list', () => {
  ...
  it('loads the full account by id before opening edit, test, and stats actions', async () => {
    ...
    menu.vm.$emit('reauth', listRow)
    await flushPromises()
    expect(getById).toHaveBeenCalledTimes(4)
    const reauth = wrapper.getComponent({ name: 'ReAuthAccountModal' })
    expect(reauth.props('account')).toEqual(fullAccount)
    expect(reauth.props('show')).toBe(true)
    wrapper.unmount()
  })
