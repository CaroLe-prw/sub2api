        ...
        >
          ...
          >
            ...
            >
              ...
              {{ t("admin.groups.modelAllowlist.batch.action", { count: selectedGroupIDs.length }) }}
            </button>
            <button
              v-if="!authStore.isSimpleMode"
              @click="openSortModal"
...
const handleBatchModelsListSave = async (modelAllowlist: ModelAllowlist) => {
  if (selectedGroupIDs.value.length === 0 || batchModelsListSaving.value) return;
  batchModelsListSaving.value = true;
  try {
    const result = await adminAPI.groups.batchSetModelAllowlist(
      selectedGroupIDs.value,
      modelAllowlist,
    );
    appStore.showSuccess(
      t("admin.groups.modelAllowlist.batch.success", { count: result.affected }),
    );
    selectedGroupIDs.value = [];
    ...
    appStore.showError(
      extractApiErrorMessage(error, t("admin.groups.modelAllowlist.batch.failed")),
    );
  } finally {
