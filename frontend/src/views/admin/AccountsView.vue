<template>
  <AppLayout>
    ...
    <TokenAuditModal :show="showTokenAudit" :account="tokenAuditAcc" @close="closeTokenAuditModal" />
    ...
    />
    <AccountActionMenu :show="menu.show" :account="menu.acc" :anchor-rect="menu.anchorRect" @close="menu.show = false" @test="handleTest" @stats="handleViewStats" @token-audit="handleTokenAudit" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @pause-scheduling="handlePauseScheduling" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
import TokenAuditModal from '@/components/admin/account/TokenAuditModal.vue'
...
const showTokenAudit = ref(false)
...
const tokenAuditAcc = ref<Account | null>(null)
...
const load = async (options: AccountLoadOptions = {}) => {
  ...
  await baseLoad()
  const refreshTasks = [loadPoolMonitorOverview()]
  if (options.refreshTodayStats !== false) refreshTasks.push(refreshTodayStatsBatch())
...
const closeTokenAuditModal = () => { showTokenAudit.value = false; tokenAuditAcc.value = null }
...
const handleTokenAudit = async (a: AccountListItem) => {
  const account = await loadAccountDetails(a)
  if (!account) return
  tokenAuditAcc.value = account
  showTokenAudit.value = true
}
