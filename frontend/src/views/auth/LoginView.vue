<template>
  <AuthLayout variant="paper">
    <div class="auth-panel space-y-7">
      <!-- Title -->
      <div class="text-center">
        <h2 class="auth-title text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.welcomeBack') }}
        </h2>
        <p class="auth-subtitle mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ t('auth.signInToAccount') }}
        </p>
      </div>
      <!-- Login Form -->
      <form class="auth-form space-y-5" @submit.prevent="handleLogin">
        <!-- Email Input -->
        <div>
        ...
        <button
          ...
          class="btn btn-primary auth-submit w-full"
        >
          <svg
            v-if="isLoading"
        ...
        <div v-if="showPasskeyLogin || showOAuthLogin" class="space-y-3 pt-1">
          ...
          <button
            ...
            type="button"
            class="btn btn-secondary auth-oauth-button w-full"
            :disabled="authActionDisabled"
