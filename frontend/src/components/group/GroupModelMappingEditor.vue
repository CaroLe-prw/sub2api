<template>
  <div class="space-y-3 border-t border-gray-200 pt-4 dark:border-dark-600" data-testid="group-model-mapping">
    <div>
      <label class="input-label">{{ t('admin.groups.modelMapping.title') }}</label>
      <p class="input-hint">{{ t('admin.groups.modelMapping.hint') }}</p>
    </div>
    <div v-for="(row, index) in modelValue" :key="index" class="flex items-center gap-2">
      <input
        ...
        required
        :aria-label="t('admin.groups.modelMapping.from')"
        :placeholder="t('admin.groups.modelMapping.from')"
      ...
      <input
        ...
        class="input min-w-0 flex-1"
        required
        :aria-label="t('admin.groups.modelMapping.to')"
        :placeholder="t('admin.groups.modelMapping.to')"
...
import type { GroupModelMappingRow } from '@/utils/groupModelMapping'

const props = defineProps<{ modelValue: GroupModelMappingRow[] }>()
const emit = defineEmits<{ 'update:modelValue': [rows: GroupModelMappingRow[]] }>()
const { t } = useI18n()
