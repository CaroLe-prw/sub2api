...
function currentModelState(model: PoolMonitorModel): CombinedMonitorHealthState {
  return summarizeCombinedHealth(model, probeHistory(model)).state
}
...
const combinedCurrentSuccesses = computed(() => props.account?.models.reduce(
  (total, model) => total + summarizeCombinedHealth(model, probeHistory(model)).successCount,
  0,
) ?? 0)
const combinedCurrentFailures = computed(() => props.account?.models.reduce(
  (total, model) => total + summarizeCombinedHealth(model, probeHistory(model)).failureCount,
  0,
) ?? 0)
...
function probeHistory(model: PoolMonitorModel): PoolProbeResult[] {
  return hasActiveProbe(model) ? history(model.plan_id) : []
}
        ...
        <div v-else class="divide-y divide-gray-100 overflow-hidden rounded-2xl border border-gray-200 dark:divide-dark-700 dark:border-dark-700">
          <article v-for="model in account.models" :key="monitorModelKey(model)" class="p-4">
            ...
            <div class="mt-3 space-y-3">
              ...
              <section v-if="hasActiveProbe(model)" class="rounded-xl bg-gray-50/80 px-3 pb-3 pt-2.5 dark:bg-dark-900/40">
                <div class="flex flex-wrap items-center justify-between gap-2 text-[11px]">
                  <span class="font-semibold text-gray-600 dark:text-gray-300">{{ t('admin.channelMonitor.dataPanel.activeProbeTimeline') }}</span>
                  <span class="text-gray-400">{{ t('admin.channelMonitor.dataPanel.timelineSampleCount', { n: probeHistory(model).length }) }}</span>
                </div>
                <MonitorHeartbeatTimeline :samples="probeHistory(model)" />
              </section>
              <p v-else class="rounded-xl bg-gray-50/80 px-3 py-3 text-[11px] text-gray-400 dark:bg-dark-900/40">{{ t('admin.channelMonitor.dataPanel.noActiveProbeForModel') }}</p>
            </div>
          </article>
        </div>
      </section>
